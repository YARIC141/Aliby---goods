// Explicit trust anchor for T-Bank (Tinkoff) API calls.
// Deno/edge-runtime's default root store does not include the Russian
// Trusted Root CA that T-Bank's certificate chain relies on, so we pass
// it explicitly to a dedicated HTTP client used for all T-Bank requests.
...
export const tbankHttpClient = Deno.createHttpClient({ caCerts: [RUSSIAN_TRUSTED_CA] })
