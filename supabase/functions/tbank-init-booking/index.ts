...
Deno.serve(async (req: Request) => {
  ...
  let body: { booking_id?: string; applied_user_subscription_id?: string }
  try { body = await req.json() } catch { return jsonResponse({ error: "Invalid JSON body" }, 400) }

  const { booking_id, applied_user_subscription_id } = body
  ...
  const { data: booking } = await serviceClient
    .from("bookings")
    .select("id, user_id, store_id, total_price, menu_item_id, slot_date, slot_start, payment_status, status, menu_items(id, category_id)")
    .eq("id", booking_id)
    .maybeSingle()
  ...
  // Оплата 100%-абонементом — без эквайринга. Раньше клиент писал
  // payment_status:'paid' напрямую в таблицу, из-за чего использование
  // абонемента никогда не списывалось и один абонемент давал
  // неограниченное число бесплатных записей.
  if (applied_user_subscription_id) {
    ...
    const item = booking.menu_items as unknown as { id: string; category_id: string | null } | null
    ...
    const eligible = us && us.user_id === user.id && us.status === "active" && us.remaining_uses !== 0 &&
      sub && sub.store_id === booking.store_id &&
      sub.discount_type === "percent" && Number(sub.discount_value) >= 100 && covered

    if (!eligible) return jsonResponse({ error: "Абонемент не подходит для оплаты этой записи" }, 400)

    const { error: payErr } = await serviceClient
      .from("bookings")
      .update({ payment_status: "paid" })
      .eq("id", booking_id)
    if (payErr) return jsonResponse({ error: "Failed to mark booking paid: " + payErr.message }, 500)
