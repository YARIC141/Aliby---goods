#!/usr/bin/env python3
"""Generate Google Play Feature Graphic 1024x500 for Alliby Business."""
...
# Business brand palette (blue, matches icon-512.png background)
C_TOP = (120, 180, 224)   # #78b4e0
C_BOT = (74, 127, 181)    # darker blue
...
# app icon tile — left side, vertically centered (reuse real icon asset)
ICON_SIZE = 200
...
try:   font_title = ImageFont.truetype(FONT_BOLD, 88)
except Exception: font_title = ImageFont.load_default()
center_text(draw, "Alliby Business", font_title, TEXT_CX, TEXT_CY - 40)

try:   font_sub = ImageFont.truetype(FONT_REG, 27)
except Exception: font_sub = ImageFont.load_default()
center_text(draw, "Управляйте заведением и заказами без комиссии", font_sub, TEXT_CX, TEXT_CY + 55,
            color=(230, 242, 252))
