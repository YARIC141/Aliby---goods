...
// Шаг аренды зала — два часа, цена считается за шаг; «1 час» в названии врал.
rename(KNTS, 'Аренда зала целиком, 1 час', 'Аренда зала целиком, 2 часа');
...
item(kTable, 'Аренда зала целиком, 2 часа', {
  desc: 'Все 11 столов, раздевалки и судейский стол. Для турниров и корпоративов. Бронь блоками по два часа.',
...
// Правило с меньшим priority выигрывает (клиент и item_rule_price сортируют по возрастанию),
// поэтому старое ручное «Будни до 18:00» (400 ₽, priority 0) перебивало дневной тариф.
sql(`DELETE FROM item_price_rules WHERE label='Будни до 18:00'
AND item_id = (SELECT id FROM menu_items WHERE store_id='${KNTS}' AND name='Теннисный стол, 1 час' LIMIT 1);`);
...
item(aVr, 'VR-зона, 30 минут', {
  ...
  price: 500, img: 'vr', type: 'rental', rentUnit: 'minute', rentStep: 30, qty: 2, hit: true });
